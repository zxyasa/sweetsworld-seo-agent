...
function generateHashtags(platform) {
    const hashtags = {
    ...
    return hashtags[platform].join(' ');
}
function generateVariantA(post, platform, utmUrl) {
...
function generateVariantB(post, platform, utmUrl) {
    const hashtags = generateHashtags(platform);
    switch (platform) {
        case 'facebook':
...
function generateSocialContent(post) {
    const utmUrl = (0, utm_1.buildUtmUrl)(post.url, 'social', post.slug, 'a'); // Base UTM, variants will adjust
