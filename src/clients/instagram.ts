...
export async function postToInstagram(caption: string, imageUrl?: string): Promise<{ status: string; response?: any }> {
  if (!config.instagram.businessId || !config.facebook.accessToken) {
  ...
  try {
    ...
    const containerResponse = await fetch(
      `https://graph.facebook.com/v18.0/${config.instagram.businessId}/media`,
      {
        method: 'POST',
        headers: {
    ...
    const publishResponse = await fetch(
      `https://graph.facebook.com/v18.0/${config.instagram.businessId}/media_publish`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
